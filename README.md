...
##Configuration
Laika uses one configuration file for itself to set up how the program logs messages, and any number of bot configuration files for loading and starting bot objects.

**NOTE:** While filling config files, keep in mind that yaml will treat `None` as a string ('None'), and not as python's `NoneType`. To specify `None`, use eitehr `nill`, or leave the value blank.
####Laika.cfg
...
An alternate or explicit config file can be specified with the `--config` argument (For more information of command arguments, see the [argument section](laika#Command Line Arguments)), so long as it is parseable by [`pyyaml`](https://github.com/yaml/pyyaml). If an alternate configuration file is not supplied at run time, Laika will look for a `laika.cfg` file in the current directory ('.'). This means it will search for whatever the current shell directory is for a `laika.cfg` file, not necesarially the laika project directory. 


######Configuration values:
* `defaultFormat`: This is the default logging format log levels will use if not explicitly provided with any of the below log format options. This, by default, is `%(asctime)s - %(name)s - %(levelname)s - %(message)s`
...
####Bot configurations
Laika will load a theortically infinte number of valid configuration files from inside `[...]/laika/config` and load each as a seperate bot object on an individual thread.

For a bot to be loaded, the configuration file must be parseable by [`pyyaml`](https://github.com/yaml/pyyaml), be loacated in `[...]/laika/config/`, and end with a `.yaml` extension.
...
* `server` : This section specifies how the bot will connect with the IRC server.
    ...
    * `pass` : If provided, the bot will pass this password value to the IRC server while connecting. **THIS PASSWORD IS NOT OBFUSCATED AND WILL REMAIN IN PLAINTEXT**. Additionally, if you are not connecting over an SSL connection to the IRC server, this password will also be sent to the server as plaintext.
* `nick` : The nick/nickname the bot will use.
* `nickPass` : If the bot catches an IRC notice from NickServ (NickServ!NickServ@services) about needing to authenticate, the bot will automatically pass this value to a function that attempts to authenticate with NickServ. If `nill`, or left blank, this authenticating function will return without attempting to authenticate the nickname with NickServ.
* `mask` : Boolean to enable or disable bot waiting for IRC mask. If `True`, the bot will withold joining any channels untill it's sent an IRC notice containing the string '`is now your hidden host (set by services.)`' so as not to leak the bot's hostname to any channels.
* `ident` : The ident the bot will use. If unsure what to set this to, use the `nick` value here again.
* `userMode` : Bot's user mode sent to the IRC server durring initial connection. If unsure what to set this to, set it to `8`.
...
* `botLog` : These values specify how the bot will handle logging it's own messages. These values share the documentation and syntax of the above values listed under [Laika.cfg](laika#Laika.cfg) configuration section. If you don't wish to bother with these, they can safely all be left blank or `nill` and they will use defaults.
* `ircLog` : This section specifies how the bot will handle logging for messages sent and recieved through the socket, as well as IRC channel messages.
    ...
    * `serverLogFile` : Name of the log that records all messages recieved from the IRC server, with the path "`logRoot`/`botLogName`/`serverLogFile`". If `nill` or left blank, will default to `server_log.log`.
    ...
    * `timeStampFormat` : How timestamps will appear. This can either be a raw string and function as a prefix, or you can supply [`strftime` formats](https://docs.python.org/3.4/library/time.html#time.strftime). It's safer to wrap this value in quotes so yaml does not attempt to parse any characters (`%`, for example) as yaml-specific. If `nill` or left blank, will deffault to `[%Y-%m-%d %H:%M:%S]`.
...
Laika can accept a small handful of command line arguments, and the complete set of program logging values can be supplied by command line arguments. If a command line argument is specified that also exists in a loaded configuration file, the command line argument will be given presidence and replace the value when creating the logs.
...
Laika has a built in help command which can print complete usage information by passing it `-h` or `--help`.
...
* `--config` : A full path to an alternate program config file. This will act as the loaded configuration file in lieu of the default `laika.cfg`. The same presedenc and replacement rules of supplied command line config values still apply.
...
* `command_str` : A matching string used to identify if the command is being called or not.
...
* `self.socket` : Accessable from a command file to utilize any of `irc.py`'s [functions]() TODO: ADD LINK.
...
* Any methods in the `commandThread()` object are accessable as well.
#####Arguments
...
```
Having done this, you can now specify arguments in the `args` field of the config by supplying (in order) the argument types you expect to recieve. 
...
* `type` : Where you specify what kind of match you are expecting.
* `optional` : Boolean value to indicate whether or not the argument is optional. If `True`, the command will fail to run if a valid match cannot be found.
...
Matches to individual arguments will be returned to the command as keyword arguments, under the name specified by the `name` option in the `Argument()` object.

To illustrate:
in the above `echo.py` configuration, a valid match for a channel name would be returned as `channel: <matched channel>` and a valid matched message would be returned as `msg: <matched message>` in keyword arguments to the command. These values can then be accessed just like any other python `**kwargs`.

#####Potential Behaviors
Each command is spawned on its own thread and will run concurrently alongside other command threads if more than one command is issued or running at the same time. There is no timeout for how long commands can exist, but there is a thread pool max size of `5` by default. If your command fatally crashes or raises an exception, the thread will die but be removed from the threadPool. However if your command results in an infinite loop that will not ever exit, fatally or otherwise, the thread will remain in the thread pool until the bot is restarted. As of version 1.0 there is no way to set the thread pool limit higher, or flush running command threads without restarting the bot.
...
When a command attempts to run, several outcomes are possible. They usually have some kind of log message or built in notification with an IRC notice, but are listed below for documentation's sake.
...
######User Not Authorized
If a user attempts to issue a command that has the `auth` key in the commands `config` dict set to `True` and their nickname is not in the `authList` list, the command will be refused and the user will be issued the following IRC notice:
...
If the `args` key in a command's `config` dict is non-empty but no *required* arguments were passed to the command, it will raise a `commandError` with the following detail string:

"`Command requested arguments but recieved none.`"
...
"`Command requested no arguments but arguments were recieved.`"

######CommandError: Regex Matching Failed
If the result of the regex match applied to the arguments results in `None` and the command expected at least one required argument to match, it will raise a `commandError` with the following detail string:
...
I can be reached at mail <at> colinjstevens <dot> com
