    ...
    def load_modules(self):
        ...
        for module in self.module_list:
            try:
                ...
                t.start()
            except AttributeError: # Run method could not be found or accessed
                continue
    def close_modules(self):
        '''Invokes a closing method on all modules for cleanup when the bot is exiting.'''
        for m in self.module_list:
            try:
                m.shutdown()
            except AttributeError: # Shutdown method could not be found
                pass
    def parse(self, line):
    ...
    def run(self):
        ...
        while self.socketWrapper.runState is True:
            try:
                ...
                self.logger.warning("Encountered a blocking error while attempting to receive data from the server.")
                continue
